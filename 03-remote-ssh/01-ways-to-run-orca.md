...
SSH worktrees can open in VS Code Remote-SSH, download remote folders when SFTP allows it, and stay usable for files/git even when the remote cannot compile terminal natives (install build tools for shells). Details: SSH worktrees and the recipe Work on a remote machine over SSH.
...
In the product UI this surface is labeled **Cloud VM** under Settings → Experimental. Recipes still create per-workspace environments.
...
1. Enable **Cloud VM** under Settings → Experimental. The pane includes a short **Create a Cloud VM** guide and recipe/runtime controls.
