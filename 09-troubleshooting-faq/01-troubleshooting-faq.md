...
Register the CLI under Settings → General → Orca CLI. On macOS it installs a shim into `~/.local/bin`; make sure that's on your shell's `PATH`.

## SSH connects but remote terminals fail

- Confirm the remote has Node and network access for the first-time relay install.
- On Linux, install a C/C++ toolchain if terminals never spawn: make, g++/clang++, python3.
...
Rate limits, bad `gh` auth, missing scopes, and repo access issues all show up in Source Control and the PR Checks panel. See the GitHub errors troubleshooting guide for comprehensive details.
...
- **Help → Send Feedback** (in-app) — paste or drag screenshots into the dialog, or pick image files; thumbnails show before you submit. Attach logs when the bug is hard to reproduce.
